...
logger = logging.getLogger("Liveblog Client")
...
class LiveblogClient(cmd.Cmd):
    ...
    def do_fetch_latest_posts(self, _):
        'Request and output the latest blog posts: fetch_latest_posts'
        if (response := self.request_get('')) is not None:
            posts = response.json()
            logger.info(posts)
    ...
    def request_post(self, url, payload={}, json=None):
        ...
        try:
            ...
            logger.info(f'Failure to connect ({error})')
            print('Request failed, check URL or server status')
            return None
